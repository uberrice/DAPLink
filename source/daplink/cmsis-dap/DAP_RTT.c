...
static uint8_t rtt_msg_buf[RTT_MSG_BUF_SIZE];
...
uint32_t readBlocks(uint16_t blocks, uint8_t * response) {
    ...
    rtt_request_buf[2] = blocks >> 8; // count
    rtt_request_buf[3] = blocks; //count
...
uint32_t connect(uint8_t * response) {
    rtt_request_buf[0] = ID_DAP_Connect;
    rtt_request_buf[1] = 0x01;
    rtt_request_buf[2] = 0x00;
    return DAP_ProcessCommand(rtt_request_buf, response);
}
